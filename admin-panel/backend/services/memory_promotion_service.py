...
def _load_proven_entries(db, workspace_id: int) -> list[dict]:
    rows = db.execute(
    ...
    for row in rows:
        try:
            ...
            findings = []
        result.append({"entry_id": row["id"], "findings": findings})
    return result
...
def _count_title_recurrences(entries: list[dict]) -> dict[str, int]:
    counts: dict[str, int] = {}
...
def _llm_approves(finding: dict) -> bool:
    title = finding.get("summary", "")
    body = finding.get("details", "")
...
def promote(db, workspace_id: int) -> dict:
    ...
    entries = _load_proven_entries(db, workspace_id)
    title_counts = _count_title_recurrences(entries)
    ...
    for entry in entries:
        for finding in entry["findings"]:
            if not _is_project_level(finding, title_counts):
                continue

            candidates_examined += 1

            try:
                approved_by_llm = _llm_approves(finding)
            ...
            try:
                duplicate = _is_duplicate(finding, project_id)
            ...
            proposal = proposal_service.create(
                ...
                payload={
                    "content": body,
                    "scope": {"kind": "project", "project_id": str(project_id)},
                    "metadata": {
                        "tags": ["research-promoted"],
                        "source_research_id": finding.get("id"),
                    },
                },
