...
- Human-facing reports go in scratchpads via the `scratchpad_create`/`scratchpad_replace`/`scratchpad_patch`/`scratchpad_delete` tools (see the `scratchpad` skill) — never files in /tmp or a sub-agent spawned just to write one.
